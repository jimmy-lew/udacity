...
from data_types import QueryBody, StatusResponse, \
    TimeResponse, TripResponse, UserResponse
from globals import MONTHS


class Parser:
    """Class to parse user query and return response
    ...
    def load_data(self, query: QueryBody) -> StatusResponse:
        ...
        city, date_filter, month, day = query['city'], query['filter'], query['month'], query['day']
        ...
        df = df[df['month'] == (MONTHS.index(month) + 1)] if self.filtered_by_month else df
        ...
        if self.filtered_by_month and len(df) == 0:
            return {
                'code': 400,
                'status': 'Bad request',
                'message': f'No data for {city} for {month}'
            }
    ...
    def get_raw_data(self, start: int, end: int) -> pd.DataFrame:
        """
        Gets raw data
        ...
        Returns:
            pd.DataFrame: Response object containing raw dataframe
        """
